...
class Peoples extends Controller
{
    ...
    {    
        ...
        $Config['nav']      = 'peoples';
        $Config['search']   = true;
        $Config['page']     = $this->translate('Peoples');
        ...
        if(isset($_POST['_ACTION']) AND $_POST['_ACTION'] == 'filter') {
           ...
           header("location: ".APP.'/admin/peoples'.$FilterSlug);
        ...
        foreach($Filter as $key => $value) {
            if (isset($_GET[$key])) {
                ...
                $i++;
                if($value == 'filter') {
                    $Where .= 'peoples.'.$key.' = "'.$_GET[$key].'" AND '; 
                } elseif ($value == 'search') {
                    $Where .= 'peoples.name LIKE "%'.$_GET[$key].'%" AND ';
                } elseif ($value == 'order') {
                    $Orderby = 'Order by peoples.id '.$_GET[$key];
                }
            } 
        }
        ...
        $Listings = $this->db->from(null,'
            ...
            ->all();
        $Pagination         = $this->db->showPagination(APP.'/admin/peoples'.(isset($FilterSlug) ? $FilterSlug : '?').'page=[page]');
