...
class Serie extends Controller
{
    ...
    {    
        ...
        if (isset($Route->params->id)) {
            ...
            // Seasons
            $Seasons    = $this->db->from('posts_season')->where('post_id', $Listing['id'])->orderby('sortable','ASC')->all();
    ...
    public function save() { 
        if (empty($Notify)) {  
            ...
            foreach ($_POST['season'] as $Season) {
                if (empty($Season['id']) AND !empty($Season['name'])) {
                    $dataarray = array(
                        "post_id"       => $post_id,
    ...
    public function update() {
        ...
        if (empty($Notify)) {
            ...
            // Seasons     
            foreach ($_POST['season'] as $Season) {
                if (isset($Season['id']) AND !empty($Season['name'])) {
                    $dataarray = array(
                        "post_id"       => $Listing['id'],
                        "name"          => Input::cleaner($Season['name']),
                        "sortable"      => (int)Input::cleaner($Season['sortable'])
                ...
                } elseif (empty($Season['id']) AND !empty($Season['name'])) {
                    $dataarray = array(
                        "post_id"       => $Listing['id'],
                        "name"          => Input::cleaner($Season['name']),
                        "sortable"      => (int)Input::cleaner($Season['sortable'])
            ...
            $Notify['text']     = 'Action completed successfully'; 
            $this->notify($Notify);
            header("location: ".APP.'/admin/serie/'.$Listing['id']);
        }else{ 
            $this->notify($Notify);
        } 
        return $this;
